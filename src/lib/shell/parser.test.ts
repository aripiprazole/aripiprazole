...
describe("parsePipeline", () => {
  ...
  test("parses quoted, concatenated, and empty arguments", () => {
    expect(
      parsePipeline(`cat '' "" pre"mid"'post' "two words" 'three words'`),
    ).toEqual({
      stages: [
        {
  ...
  test("supports backslash escaping outside and inside double quotes", () => {
    expect(
      parsePipeline(String.raw`cat one\ two \| \$ "say \"hello\""`),
    ).toEqual({
  ...
  test("keeps metacharacters literal when quoted or escaped", () => {
    expect(
      parsePipeline(String.raw`cat '>$*?[{}]();&|' "literal > value" \>`),
    ).toEqual({
  ...
  test("rejects a ninth pipeline stage", () => {
    const source = Array.from(
      { length: shellParserLimits.stages + 1 },
      () => "cat",
    ).join("|");
