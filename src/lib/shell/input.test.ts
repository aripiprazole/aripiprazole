...
const labels = async (source: string, cursor = source.length): Promise<string[]> =>
...
describe('shell completion', () => {
	test('suggests commands at an empty prompt and after a pipeline', async () => {
		expect(await labels('')).toEqual(['cat', 'cd', 'curl', 'ls', 'pwd', 'split']);
	...
	test('filters virtual paths by command operand kind', async () => {
		...
		expect(ls).toEqual(expect.arrayContaining(['README.md', 'projects/', '--all']));
	});
	...
	test('preserves the surrounding pipeline when completing at the caret', async () => {
		...
		const candidates = await completeShellInput(source, 6, filesystem.initialDirectory);
		const readme = candidates.find((candidate) => candidate.label === 'README.md');
