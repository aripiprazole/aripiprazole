<script lang="ts">
	...
	const initialPanelId = createPanel(undefined, [
		'cat README.md',
		'ls -la .'
	]);
	let layout = $state<Layout>(panelLeaf(initialPanelId));
	let activePanelId = $state<PanelId>(initialPanelId);
	...
	let draggingPanelId = $derived(pointerDrag?.active ? pointerDrag.sourceId : null);
	let dropTargetPanelId = $derived(pointerDrag?.active ? pointerDrag.targetId : null);
	...
	const cancelPointerDrag = (): void => {
		...
		if (
			current !== null &&
			dragHandle?.hasPointerCapture(current.pointerId)
		) {
	...
	const dragPointerMove = (event: PointerEvent): void => {
		...
		pointerDrag = {
			...
			active: true,
			targetId:
				target !== undefined && target !== current.sourceId && controllers.has(target)
					? target
					: null
		};
	};
